...
- **Known unfixable limitation — bottom safe-area strip on iOS Home Screen**: on a real iPhone (confirmed on iPhone 16 Pro, standalone "Add to Home Screen" launch), `window.innerHeight`/`visualViewport.height`/`document.documentElement.clientHeight` are permanently capped at 812px while `screen.height` correctly reports 874px — a ~62pt strip at the bottom that the page cannot draw into. This was confirmed via an on-device debug overlay (live-updating `getBoundingClientRect`/viewport readout) across many attempted fixes: `100dvh`, JS-measured `--app-height` custom property, fixing a real `PROD`-flag-read-too-early race condition (the bundler's async script-recreation could read `window.CAVE_PRODUCTION` before it was set — now read lazily via a function, not a one-time `const`), a full `manifest.json` with `display:standalone`, adding `viewport-fit=cover`/`apple-mobile-web-app-*` meta tags to the **real** initially-parsed `<head>` (they'd only existed in the JS-injected `__bundler/template` before, which iOS ignores for the initial viewport/web-clip contract), the documented "iOS keyboard shrinks viewport permanently" workaround (toggling `display` on blur to force a re-measure), and a device-matched `apple-touch-startup-image`. None moved the number by a single pixel, including after several genuine icon delete-and-recreate cycles (required since some of these are only read at web-clip creation time). Deliberately extending an element past the measured 812px boundary via negative offset also failed to paint further — proving the canvas is truly hard-clipped by iOS for ad-hoc web clips, not a measurement-only quirk. **Conclusion: this is a hard platform limit for "Add to Home Screen" web clips (as opposed to real App Store apps), not fixable from web content.** Current approach: don't fight it — keep the reserved strip the same background color as the rest of the app (`#F6F2EB`) so it reads as intentional margin rather than a visible defect. Don't re-attempt any of the above fixes without new evidence; if revisiting, the fastest way back to ground truth is to re-add the live debug overlay pattern (see git history around commits `6b0eed9`–`deac205a` for the exact approach) rather than guessing blind.
